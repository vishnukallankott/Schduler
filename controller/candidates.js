...
const addCandidate = async (req, reply) => {
    try {
        ...
        const details = await userModel.findOne({ where: { email: email } })
        if (details) {
        ...
        if (response.messageId) {
            await userModel.update({ sendflag: true }, {
...
const reschduleCandidates = async (req, reply) => {
    try {
        ...
        const updateUser = await userModel.update({ schduledtime: schduledtime }, { where: { id: req.params.id } })
...
const updateCandidateDetails = async (req, reply) => {
    try {
        ...
        const updateUser = await userModel.update(userData, { where: { id: req.params.id } })
        reply.status(statusCodes.status.OK).send({
            status: true,
            message: updateUser,
            error: []
...
const deleteCandidate = async (req, reply) => {
    try {
        const { id } = req.params
        console.log(id)
        const userDetails = await userModel.findOne({ where: { id: id } })
        if (!userDetails) {
            return reply.status(statusCodes.status.NOTFOUND).send({
                status: false,
                message: statusCodes.message.NOTFOUND,
        ...
        const DeleteDetails = await userModel.destroy({ where: { id: id } })
        if (!DeleteDetails) {
...
const getCandidates = async (req, reply) => {
    try {
        ...
        const details = await userModel.findAll(condition)
